    ...
    useEffect(() => {
        const func = async () => {
            ...
            if (memoized.current !== collection) {
                ...
                internalSetState(stored ?? { pagination: { pageIndex: 0, pageSize: 20 }} as JITTTableState<T>);
                setIsNotLoading();
            }
    ...
    useEffect(() => {
        ...
        token.current = setImmediate(async () => {
            ...
            if (!deepEqual(stored, state)) {
                ...
                fs.appendFileSync('C:/Users/bobby/OneDrive/Desktop/settings.log', [collection, JSON.stringify(state, null, '\t')].join(': ').concat('\n'));
            } else {
