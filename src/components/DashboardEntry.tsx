...
export function BaseDashboardEntry({ label, start, limit, value, ...rest }: { label: string, start?: number, limit: 'overlimit' | 'underlimit', value: string, className?: string }) {
    const { className } = useMemo(
        () =>
            $className(
                rest,
    ...
    return (
        <>
            <Item className={classes}>
                <span className={className}>{label.concat(' :')}</span>
            </Item>
            <Item className='flex justify-start'>
                <span
                    className='ml-5 rounded-md border border-cyan-500 bg-black p-0.5 px-3 text-lg font-bold text-white shadow-inner shadow-white overlimit:border-pink-300 overlimit:bg-red-500 overlimit:text-white'
                    data-value={limit}>
                    {value}
                </span>
            </Item>
