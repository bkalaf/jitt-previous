...
export function colEnum<T extends MRT_RowData>(helper: MRT_ColumnHelper<T>) {
    return function <TKey extends keyof T>(...dependencies: IDependency<T, TKey>[]) {
        return function (name: keyof T & string, $header: string, opts: { id?: string, options?: Record<string, string | { text: string; key: string }>; required?: boolean; readonly?: boolean; enumKey?: keyof typeof $me }): MRT_ColumnDef<T, string | undefined> {
