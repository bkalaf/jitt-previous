...
export function clothingCareColumnDefinition<T extends MRT_RowData>(helper: MRT_ColumnHelper<T>) {
    return function <TKey extends keyof T>(...dependencies: IDependency<T, TKey>[]) {
        return function (name: Path<T> & string, header: string, section: keyof typeof ClothingCareMap, readonly = false): MRT_ColumnDef<T> {
            return baseCol<T, ListBack<string>>(helper, name, FlattenedClothingCare, ClothingCareControl, header, false, readonly, {}, undefined, ...dependencies) as MRT_ColumnDef<T>;
