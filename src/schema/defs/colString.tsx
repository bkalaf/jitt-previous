...
import { StringControl, TextAreaControl } from '../../components/table/controls/StringControl';
...
export function colString<T extends MRT_RowData, U>(helper: MRT_ColumnHelper<T>) {
    return function (
        ...
        return baseCol<T, U | undefined>(helper, name, StringTableCell, StringControl, $header, required, readonly, { ...rest, formatter: $formatter });
...
export function colText<T extends MRT_RowData, U>(helper: MRT_ColumnHelper<T>) {
    return function (
        ...
        const { required, readonly, ...rest } = { readonly: false, required: false, ...(opts ?? {}) };
        const $formatter = formatter ?? ((x?: U) => (x ?? '') as string);
        return baseCol<T, U | undefined>(helper, name, StringTableCell, TextAreaControl, $header, required, readonly, { ...rest, formatter: $formatter });
    };
}
